...
scData_collapse<-filterCells(scData_collapse,minimumSegments = 40,minDensity = 0.05)
...
# write.table(nmf_results$clusterNormal,file=str_c(scCNVCaller$locPrefix,scCNVCaller$outPrefix,"_nmf_normal.csv"),quote=FALSE,row.names = FALSE,sep=",")
cat(nmf_results$clusterNormal, file=str_c(scCNVCaller$locPrefix,scCNVCaller$outPrefix,"_nmf_normal.csv"))
...
barcodeCycling<-estimateCellCycleFraction(scData,sampName="sample",cutoff=10)
#take max as 
